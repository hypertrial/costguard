...
Pin a version: `... | sh -s -- v2.7.0`. Or build from source: `cargo install --git https://github.com/hypertrial/costguard --tag v2.7.0 costguard-cli`.
...
Pin the exact Action tag `@v2.7.0` or use the moving compatible major tag `@v2`. Release binaries are checksum-protected and include provenance attestations.
...
`v2.7.0` adds first-class sealed Rocky analysis, mixed dbt/Rocky lineage, exact-base Rocky comparison, a shared framework snapshot pipeline, and run-wide immutable-base budgeting. JSON schema v4, receipt v2, Rocky artifact v1, baseline v3, policy v2, dbt APIs, rule IDs, configuration syntax, and Action inputs remain compatible. Generic SQL, Snowflake, BigQuery, and Trino are supported; Databricks, Redshift, Postgres, and DuckDB remain preview. Cost estimates are advisory, warehouse connectivity is out of scope, and framework compilation remains the caller's responsibility. See the [support policy](SUPPORT.md), [compatibility policy](docs/book/reference/compatibility.md), and [security policy](SECURITY.md).
