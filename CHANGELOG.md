...
- Documentation: corrected rule-count prose from 44 to 45 in the SQLCOST catalog; clarified that the cost-pattern census covers `SQLCOST001`–`SQLCOST044` (`SQLCOST045` validated by integration tests). `scripts/check_docs.py` now asserts `N SQLCOST rules` claims match `docs/rules/SQLCOST*.md` count.
