...
class PublicVersionPinTest(unittest.TestCase):
    def check_text(self, text: str, current: str = "2.7.0") -> list[str]:
        ...
        errors = self.check_text(
            "\n".join(
                [
                    ...
                    "cargo install --git https://github.com/hypertrial/costguard --tag v2.7.0 costguard-cli",
                    "curl -fsSL https://example.invalid/install.sh | sh -s -- v2.7.0",
                    "VERSION=v2.7.0",
                    'COSTGUARD_VERSION: "v2.7.0"',
                    "COSTGUARD_VERSION = 'v2.7.0'",
                    "rev: v2.7.0",
                ]
            )
    ...
    def test_stale_public_pins_fail(self) -> None:
        ...
        self.assertTrue(all("does not match workspace version v2.7.0" in error for error in errors))
