...
The census covers the 44 cost/behavioral rules (`SQLCOST001`–`SQLCOST044`). `SQLCOST045` (stale dbt manifest), `SQLCOST046` (manifest checksum mismatch), and `SQLCOST047` (Rocky artifact integrity) are excluded like infrastructure rules `SQLCOST023`–`027` and are validated by integration tests.
