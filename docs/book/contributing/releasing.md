...
The current public docs pin `v2.5.0`. Treat the release PR as the `v2.5.0` release commit: after merge, create the signed annotated tag and publish verified assets in the same release window before announcement or support handoff.
...
4. Use Matt's existing passphrase-protected `~/.ssh/id_ed25519` key to create signed annotated `v2.5.0`. Do not add another key or change global Git configuration.
...
git -c gpg.format=ssh \
  ...
  tag -s -m "Costguard v2.5.0" v2.5.0
