...
The current public docs pin `v2.7.0`. Treat the release PR as the `v2.7.0` release commit: after merge, create the signed annotated tag and publish verified assets in the same release window before announcement or support handoff.
...
5. Use Matt's existing passphrase-protected `~/.ssh/id_ed25519` key to create signed annotated `v2.7.0`. Do not add another key or change global Git configuration. The tag workflow independently verifies both exact-SHA runs before publishing.
...
8. Never replace an exact release. Publish post-GA fixes as `2.7.1` and move `v2` only after verification.
...
git -c gpg.format=ssh \
  ...
  tag -s -m "Costguard v2.7.0" v2.7.0
