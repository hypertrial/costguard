...
4. Create and verify a signed annotated `v1.0.0` tag at the merge commit.
5. Run `python3 scripts/release_check.py --version 1.0.0` to create `dist/release/release-check.json`.
6. Run `./scripts/publish_release_local.sh --package-only --version 1.0.0` to build deterministic assets and local smoke receipts.
7. Transfer the MSVC archive to Windows, run `scripts/smoke_release_asset.py`, and return its receipt unchanged.
8. Run `./scripts/publish_release_local.sh --publish --version 1.0.0 --receipt PATH_TO_WINDOWS_RECEIPT`.
