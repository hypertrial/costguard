...
curl -fsSL .../install.sh | sh -s -- v2.3.0
...
Cost-ranked FP-elimination census across benchmark repos. For each behavioral rule, examines up to **100** findings (100% if fewer), ranked by `cost_estimate.savings_p50_usd_per_month`. **PASS** when the examined sample has **0 `fp_bug` and 0 `unknown`** (`tp` and documented `exempt` findings are both acceptable). Infrastructure rules SQLCOST023–027 and SQLCOST045 auto-pass.
